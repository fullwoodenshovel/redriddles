...
impl UserInputs {
    ...
    pub fn update(&mut self) {
        ...
        self.hover_focus = origin_borrow.borrow().node.hit_detect(self.mouse);
...
fn temp_node() -> Box<dyn Node + 'static> {
    Box::new(NoNode)
}
...
impl NodeStore {
    ...
    fn new<Child: Node + New + 'static>(data: Child::InType, parent: Option<WeakNode>) -> StrongRef<Self>{
        ...
        result.borrow_mut().node = Box::new(Child::new(data, weak));
    ...
    pub fn try_get_self<T: Node + 'static>(&self) -> Option<&T> {
        self.node.as_ref().as_any().downcast_ref()
    }
    
    pub fn try_get_self_mut<T: Node + 'static>(&mut self) -> Option<&mut T> {
        self.node.as_mut().as_any_mut().downcast_mut()
    }
