    ...
    fn update(&mut self, user_inputs: &UserInputs) {
        ...
        if hoverhold && hover {
            ...
            if user_inputs.left_mouse_down && hoverhold && hover {
                ...
                if let Some(col) = self.on_picker_mut(|d| d.get_col_rgba()) {
...
impl DrawSection {
    pub fn on_picker<T>(&self, f: impl FnOnce(&PickerNode) -> T) -> T {
        f(self.node.upgrade().unwrap().borrow().get_parent().borrow().children[1].borrow().children[1].borrow().get_self::<PickerNode>())
    }

    pub fn on_picker_mut<T>(&self, f: impl FnOnce(&mut PickerNode) -> T) -> T {
        f(self.node.upgrade().unwrap().borrow().get_parent().borrow().children[1].borrow().children[1].borrow_mut().get_self_mut::<PickerNode>())
    }
}