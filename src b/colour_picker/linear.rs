...
impl ColPicker for Linear {
    ...
    fn transfer_picker(self, pickertype: PickerSelection) -> Picker {
        match pickertype {
            PickerSelection::Circular => Picker::Circular(Circular::with_col(self.height / 2.0, self.width, self.offset, self.padding, self.coltype, self.selected)),
            PickerSelection::Linear => Picker::Linear(self),
        }
    }
}