...
impl ColPicker for Circular {
    ...
    fn transfer_picker(self, pickertype: PickerSelection) -> Picker {
        match pickertype {
            PickerSelection::Circular => Picker::Circular(self),
            PickerSelection::Linear => Picker::Linear(Linear::with_col(self.radius * 2.0, self.width, self.offset, self.padding, self.coltype, self.selected)),
        }
    }
}