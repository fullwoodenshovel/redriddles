...
use crate::{colour::Col, colour_picker::picker::Circular, node::{New, Node, StrongRef, UserInputs, WeakNode}};
...
impl New for Linear {
    ...
    fn new((height, width, offset, padding, coltype, selected): Self::InType, _: WeakNode) -> Self {
        Self {
            offset,
            height,
            width,
...
impl Linear {
    pub fn with_col(height: f32, width: f32, offset: [f32;2], padding: f32, coltype: StrongRef<ColSelection>, selected: StrongRef<Option<Col>>) -> Self {
        Self {
            offset,
            height,
            width,
...
impl ColPicker for Linear {
    ...
    fn draw(&self) {
        ...
        for value in 0..iheight {
            ...
            draw_rectangle(x, y, self.width, 1.0, coltype.col_from_wheel(circular, radial, 1.0 - value * div_1_height).to_macroquad_col());
        }
    ...
    fn transfer_picker(self, pickertype: PickerSelection) -> Box<dyn ColPicker> {
        match pickertype {
            PickerSelection::Circular => Box::new(Circular::with_col(self.height / 2.0, self.width, self.offset, self.padding, self.coltype, self.selected)),
            PickerSelection::Linear => Box::new(self),
        }
    }
}