...
use crate::{colour::Col, colour_picker::linear::Linear, node::{New, Node, StrongRef, UserInputs, WeakNode}};
use super::{ColSelection, ColPicker, Picker, PickerSelection};
...
pub struct Circular {
    ...
    pub width: f32,
    pub padding: f32,
    pub selected: StrongRef<Option<Col>>,
    pub coltype: StrongRef<ColSelection>,
    cache: Option<[f32; 4]>
}

impl New for Circular {
    type InType = (f32, f32, [f32;2], f32, StrongRef<ColSelection>, StrongRef<Option<Col>>);
    fn new((radius, width, offset, padding, coltype, selected): Self::InType, _: WeakNode) -> Self {
        Self {
            offset,
            radius,
            width,
...
impl Circular {
    pub fn with_col(radius: f32, width: f32, offset: [f32;2], padding: f32, coltype: StrongRef<ColSelection>, selected: StrongRef<Option<Col>>) -> Self {
        Self {
            offset,
            radius,
            width,
...
impl ColPicker for Circular {
    ...
    fn detect(&mut self, mouse: Vec2, first_mouse_down: Vec2) {
        ...
        let coltype = self.coltype.borrow();
        ...
        if slider_rect.contains(first_mouse_down - offset) {
            ...
            match &*self.selected.borrow() {
                Some(col) => {
                    ...
                    *self.selected.borrow_mut() = Some(coltype.col_from_wheel(circular, radial, scalar))
                },
                None => *self.selected.borrow_mut() = Some(coltype.col_from_wheel(coltype.default_cirular(), coltype.default_radial(), scalar))
            }
            ...
            match &*self.selected.borrow() {
                ...
                None => *self.selected.borrow_mut() = Some(coltype.col_from_wheel(circular, radial, coltype.default_scalar()))
            }
        }
    }
    ...
    fn draw(&self) {
        let coltype = self.coltype.borrow();
        let wheel_selected = self.selected.borrow().as_ref().map(|col| col.to_wheel());
        let (circular, radial, scalar) = if let Some((circular, radial, scalar)) = wheel_selected {
            (circular, radial, scalar)
        } else {
            (coltype.default_cirular(), coltype.default_radial(), coltype.default_scalar())
        };
        ...
        for x in -iradius..iradius {
            for y in -iradius..iradius {
                ...
                draw_rectangle(
                    ...
                    1.0,
                    1.0,
                    coltype.col_from_wheel(circular, radial, scalar).to_macroquad_col()
                );
            }
        }
        ...
        for value in 0..height {
            ...
            draw_rectangle(x, y, self.width, 1.0, coltype.col_from_wheel(circular, radial, 1.0 - value / max_value).to_macroquad_col());
        }
    ...
    fn get_col_rgba(&mut self) -> Option<[f32; 4]> {
        if self.cache.is_none() { self.cache = self.selected.borrow().as_ref().map(|d| d.to_rgba()) }
        self.cache
    }

    fn set_col(&mut self, col: Option<[f32; 4]>) {
        self.cache = col;
        *self.selected.borrow_mut() = col.map(|d| self.coltype.borrow().col_from_rgba_arr(d));
    }

    fn transfer_col(&mut self, coltype: ColSelection) {
        if coltype == *self.coltype.borrow() { return; };
        *self.selected.borrow_mut() = self.selected.borrow().as_ref().map(|d| coltype.col_from_rgba_arr(d.to_rgba()));
        *self.coltype.borrow_mut() = coltype;
    }

    fn transfer_picker(self, pickertype: PickerSelection) -> Box<dyn ColPicker> {
        match pickertype {
            PickerSelection::Circular => Box::new(self),
            PickerSelection::Linear => Box::new(Linear::with_col(self.radius * 2.0, self.width, self.offset, self.padding, self.coltype, self.selected)),
        }
    }
}